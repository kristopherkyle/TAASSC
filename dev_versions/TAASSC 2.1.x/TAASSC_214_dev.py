...
version = "2.1.14"
version_notes = (
    "2.1.14 adds the ability to recalculate feature counts from edited xml files"
)
...
DEPENDENCY_TAG_SET = set(nlp.get_pipe("parser").labels)
...
TAG_SET = set(nlp.get_pipe("tagger").labels)
...
PERSONAL_PRONOUN3 = (
    "he she they their his them her him themselves himself herself".split(" ")
...
SEMANTIC_ADVERB_CATEGORIES = (
    "attitudinal_adverb factive_adverb likelihood_adverb nonfactive_adverb".split(" ")
)
...
SEMANTIC_THAT_VERB_CATEGORIES = (
    "nonfactive_verb attitudinal_verb factive_verb likelihood_verb".split(" ")
)
SEMNATIC_THAT_NOUN_CATEGORIES = (
    "nn_nonfactive nn_attitudinal nn_factive_noun nn_likelihood"
)
...
def read_category_file(file_path):
    ...
    with open(file_path, "r") as category_file:
        ...
        if curr_dict != {}:
            multiple_dictionaries.append(curr_dict)
        return (
            multiple_dictionaries[0]
            if len(multiple_dictionaries) == 1
            else multiple_dictionaries
        )


NOUN_DICT = read_category_file(SEMANTIC_NOUN_FILE)
VERB_DICT, THAT_VERB_DICT, TO_VERB_DICT, PHRASAL_VERB_DICT = read_category_file(
    SEMANTIC_VERB_FILE
)
...
##########################################
cat_to_tag = {
    "main_tag": "nn_all prep_phrase verb pp_all wh_relative_clause".split(
        " "
    ),  # create list of possible main tags
...
assert all(
    set(cat_to_tag[cat1]).isdisjoint(cat_to_tag[cat2]) for cat1, cat2 in list(cat_pairs)
)
...
for (
    x
) in "main_tag spec_tag1 spec_tag2 spec_tag3 spec_tag4 spec_tag5 spec_tag6 semantic_tag1 semantic_tag2".split(
    " "
):
    cats[x] = None
...
def noun_phrase_complexity(token, feature_dict):
    if (
        token.pos_ == "NOUN"
    ):  # only consider common nouns (exclude pronouns and proper nouns)
        feature_dict["np"] += 1
...
def clausal_complexity(token, feature_dict):
    if token.pos_ == "VERB":
        if token.dep_ != "aux":  # check to make sure that this is a main verb
            feature_dict[
                "all_clauses"
            ] += 1  # finite and nonfinite clauses. Probably won't use
            deps = [child.dep_ for child in token.children]
            if "nsubj" in deps or "nsubjpass" in deps:
                ...
                feature_dict[
                    "nonfinite_clause"
                ] += 1  # can use "to_clause" to distinguis
...
def pronoun_analysis(
    token, token_d, feature_dict
):  # takes spaCY token object and feature_dict as arguments
    token_lower = token.text.lower()
    # NOTE: Spacy tags "our" and "my" as determiners - run with out tags
    pp_all = (
        PERSONAL_PRONOUN1 + PERSONAL_PRONOUN2 + PERSONAL_PRONOUN3 + PERSONAL_PRONOUN_IT
    )
    if token_lower in pp_all:
        feature_dict[
            "pp_all"
        ] += 1  # add one to feature dict (this is external to the function)
...
def advanced_pronoun(token, doc, token_d, feature_dict):  # updated 5-8-2020
    token_lower = token.text.lower()
    if token_lower in INDEFINITE_LIST and token.dep_ in [
        "nsubj",
        "nsubjpass",
        "dobj",
        "pobj",
        ...
        elif token.i + 1 < len(doc) and doc[token.i + 1].text.lower() in [
            "who",
            ".",
            "!",
            "?",
    ...
    if token.dep_ == "mark":
        ...
        for x in token.children:
            ...
            if x.dep_ in ["pobj", "pcomp", "prep", "amod", "cc"]:
                obj = True
            if (
                x.dep_ == "punct" and x.text not in ". , ? !"
            ):  # this deals with some weirdness in spacy where some numbers and mathematical symbols (e.g., "x" and "y") were not counted as prepositional objects.
                obj = True
        # Updated 5-25-20
        if (
            obj == True
        ):  # check to make sure that there is a prepositional object or another prepositional phrase
    ...
    if token.text.lower() in ["and", "or"]:  # only consider and/or
        if wrd_count == 0:
            ...
            for (
                ...
                token.head.children
            ):  # iterate the children of the main verb (1st verb)
                # print(child.text, child.i, child.dep_) #for debugging
                if child.dep_ == "cc":
                    l.append([child.i, "cc"])  # add cc index to list
                if (
                    child.dep_ == "conj"
                ):  # narrow down the target of analysis to the second verb(s) by specifying the analysis to corrdinated second element

                    if "nsubj" in [
                        chld.dep_ for chld in child.children
                    ]:  # identify if the second element has "nsubj" tag in its children, if so, this is independent clause coordination
                        l.append(
                            [child.i, "cc_clause"]
                        )  # add index and coordination type to list
                    elif "nsubj" not in [
                        chld.dep_ for chld in child.children
                    ]:  # if not it is phrasal
                        l.append(
                            [child.i, "cc_phrase"]
                        )  # add index and coordination type to list
            ...
            sort_list = sorted(
                l, key=lambda x: x[0]
            )  # sort list by index to put in order of appearance
            for x in range(
                len(sort_list)
            ):  # iterate through list of numbers that is the same length as list
                if (
                    sort_list[x][0] == token.i
                ):  # if the list item has the same index as our target token:
                    try:
                        ...
                        feature_dict[
                            relation
                        ] += 1  # add one to the count for the next token in the list (i.e., a verb attribute)
                        token_d[
                            "spec_tag1"
                        ] = relation  # set attribute as next token's attribute (this will be a verb)
                        break
                    except IndexError:
                        continue
...
def noun_analysis(token, token_d, feature_dict):  # revised 5/8/20.
    if token.pos_ in NOUN_TAGS:
        ...
        if token_lower not in NOMINAL_STOP:
            if token.pos_ == "PROPN":
                for l in range(
                    min(len(token_lower) - 1, PROPER_NOUN_NOMINALIZATION_MAX),
                    PROPER_NOUN_NOMINALIZATION_MIN - 1,
                    -1,
                ):
                    if token_lower[-l:] in PROPER_NOUN_NOMINALIZATION_BY_LENGTH[l]:
                        ...
                        break
            for l in range(
                min(len(token_lower) - 1, NOUN_NOMINALIZATION_MAX),
                NOUN_NOMINALIZATION_MIN - 1,
                -1,
            ):
                if token_lower[-l:] in PROPER_NOUN_NOMINALIZATION_BY_LENGTH[l]:
                    feature_dict["nominalization"] += 1
...
def semantic_analysis_noun(token, token_d, feature_dict):
    if token.pos_ in NOUN_TAGS:
        ...
        if (
            lemma in NOUN_DICT and NOUN_DICT[lemma] in SEMANTIC_NOUN_CATEGORIES
        ):  # check that word is in dict and category is in var_list
...
def verb_analysis(
    token, doc_text, token_d, feature_dict
):  # need to add spearate tags for tense/aspect and passives
    if token.pos_ == "VERB":
        ...
        if token.dep_ == "aux":  # check for auxilliaries
            ...
            if (
                token.head.lemma_ in THAT0_LIST
                ...
                for x in token.children:
                    if (
                        ...
                        and x.dep_ != "det"
                    ):  # this will likely be overly disriminitory
                        that0_problem = True
                    if x.dep_ == "mark":
                        that0_problem = True
                ...
                if token.tag_ == "VBG" and aux_be == False:
                    vbg_problem = True
                if that0_problem == False and doc_text[token.head.i - 1].text not in [
                    "that",
                    "who",
                    "what",
                    "how",
                    ...
                    if doc_text[token.head.i + 1].text not in [
                        "that",
                        "who",
                        "what",
                        "how",
            ...
            if (
                doc_text[token.i - 1].text.lower() == "to"
                and doc_text[token.i - 1].dep_ == "aux"
                and doc_text[token.i - 1].head.i == token.i
            ):
                contr_token = doc_text[token.i - 2]
                if contr_token.text.lower() not in [
                    "able",
                    "ought",
                ]:  # spacy wasn't getting all of the phrasal verbs (that tag "to" as "part" instead of "aux") More may need to be added here
                    feature_dict["to_clause"] += 1  # add one to count
                    token_d["spec_tag4"] = "to_clause"

                    if (
                        contr_token.pos_ == "NOUN"
                    ):  # consider using head.text instead (previous version used that)
                    ...
                    if contr_token.pos_ == "VERB":
                        ...
                        if (
                            contr_token.lemma_ in TO_VERB_DICT
                            and TO_VERB_DICT[contr_token.lemma_] in TO_VERB_LIST
                        ):
                            feature_dict[
                                "to_clause_verb_"
                                + TO_VERB_DICT[contr_token.lemma_][:-5]
                            ] += 1
                            token_d["semantic_tag2"] = (
                                "to_clause_verb_"
                                + TO_VERB_DICT[contr_token.lemma_][:-5]
                            )
                    ...
                    if doc_text[token.i - 2].pos_ == "ADJ":
                        ...
                        if (
                            contr_token.lemma_ in ADJECTIVE_DICT
                            and ADJECTIVE_DICT[contr_token.lemma_] in TO_ADJECTIVE_LIST
                        ):
                            feature_dict[
                                "to_clause_adjective_"
                                + ADJECTIVE_DICT[contr_token.lemma_][:-4]
                            ] += 1
                            token_d["semantic_tag2"] = (
                                "to_clause_adjective_"
                                + ADJECTIVE_DICT[contr_token.lemma_][:-4]
...
def passive_analysis(token, token_d, feature_dict):
    if token.pos_ == "VERB":
        ...
        for (
            ...
            token.children
        ):  # add dependents to the list - annoying, but necessary because of the way spaCy works
...
def semantic_analysis_verb(token, token_d, feature_dict):
    ...
    if token.pos_ == "VERB":
        ...
        if "prt" in [
            chld.dep_ for chld in token.children
        ]:  # first, check for phrasal_verbs
            for x in token.children:  # then extract particle text
                if x.dep_ == "prt":
                    phrasal = (
                        lemma + " " + x.text
                    )  # create phrasal verb - will only work with two-word phrasal verbs
                    # print(phrasal)
                    ...
                    if "dobj" in [
                        chld.dep_ for chld in token.children
                    ]:  # check for transitivitivity
                        if (
                            phrasal in PHRASAL_VERB_DICT
                            and PHRASAL_VERB_DICT[phrasal]
                            in TRANSITIVE_PHRASAL_CATEGORIES
                        ):
                            feature_dict[
                                PHRASAL_VERB_DICT[phrasal]
                            ] += 1  # if so, add one to feature_dict
                            token_d["semantic_tag1"] = PHRASAL_VERB_DICT[
                                phrasal
                            ]  # set attribute
                    else:
                        if (
                            phrasal in PHRASAL_VERB_DICT
                            and PHRASAL_VERB_DICT[phrasal]
                            in INTRANSITIVE_PHRASAL_CATEGORIES
                        ):
                            feature_dict[
                                PHRASAL_VERB_DICT[phrasal]
                            ] += 1  # if so, add one to feature_dict
            ...
            if (
                lemma in VERB_DICT and VERB_DICT[lemma] in SEMANTIC_VERB_CATEGORIES
            ):  # check that word is in dict and category is in var_list
...
def adjective_analysis(token, token_d, feature_dict):
    ...
    if token.dep_ in [
        "acomp"
    ]:  # if dep relation is adjective complement, clausal complement, or object predicate (see Biber et al. 1999)
        feature_dict["jj_predicative"] += 1
        token_d["spec_tag1"] = "jj_predicative"
        if (
            token.lemma_.lower() in ADJECTIVE_DICT
            and ADJECTIVE_DICT[token.lemma_.lower()] in SEMANTIC_ADJECTIVE_CATEGORIES
        ):
...
def adverb_analysis(token, w_count, token_d, feature_dict):
    ...
    if token.pos_ == "ADV" or token.dep_ in ["npadvmod", "advmod", "intj"]:
        ...
        if (
            w_count == 0
            and token.text.lower() in "well now anyway anyhow anyways".split(" ")
        ):
...
def wh_analysis(token, w_count, doc_text, sent_doc, token_d, feature_dict):
    if token.tag_ in ["WDT", "WP", "WP$", "WRB"] and token.text.lower() != "that":
        ...
        if (
            doc_text[token.i - 1].pos_ == "VERB"
            and doc_text[token.i - 1].lemma_ != "be"
        ):
...
def that_analysis(token, doc_text, token_d, feature_dict):
    ...
    if token.text.lower() == "that":
        if (
            token.dep_ in ["nsubj", "nsubjpass", "dobj", "pobj"]
            and token.head.dep_ == "relcl"
        ):  # consider adding "mark" to the possible token.dep_ options
        ...
        if token.dep_ in ["mark", "nsubj"] and token.head.dep_ in ["ccomp", "acl"]:
            ...
            if doc_text[token.i - 1].pos_ == "VERB":
                ...
                if (
                    verb_lemma in THAT_VERB_DICT
                    and THAT_VERB_DICT[verb_lemma] in SEMANTIC_THAT_VERB_CATEGORIES
                ):  # check for semantic class
                    feature_dict[
                        "that_verb_clause_" + THAT_VERB_DICT[verb_lemma][:-5]
                    ] += 1
                    token_d["semantic_tag1"] = (
                        "that_verb_clause_" + THAT_VERB_DICT[verb_lemma][:-5]
                    )
            ...
            if doc_text[token.i - 1].pos_ == "NOUN":
                ...
                # print(noun_lemma)
                if (
                    noun_lemma in NOUN_DICT
                    and NOUN_DICT[noun_lemma] in SEMNATIC_THAT_NOUN_CATEGORIES
                ):
                    # print(noun_lemma,NOUN_DICT[noun_lemma])
                    feature_dict["that_noun_clause_" + NOUN_DICT[noun_lemma][3:]] += 1
                    token_d["semantic_tag1"] = (
                        "that_noun_clause_" + NOUN_DICT[noun_lemma][3:]
                    )
...
def LGR_Analysis(text, indices_dict=INDEX_LIST, cats_d=cats, output=False):
    ...
    for sent in doc.sents:
        output_list.append(
            []
        )  # add empty sentence-level list that will be filled below
    ...
    index_dict["mean_nominal_deps"] = safe_divide(
        index_dict["np_deps"], index_dict["np"]
    )  # nominal (common noun) dependents,#of nominals (common nouns)
    ...
    index_dict["mean_verbal_deps"] = safe_divide(
        index_dict["vp_deps"], index_dict["finite_clause"]
    )  # dependents per finite clause
    index_dict["mlc"] = safe_divide(
        index_dict["nwords"], index_dict["finite_clause"]
    )  # number of words,number of finite clauses
    index_dict["mltu"] = safe_divide(
        index_dict["nwords"], index_dict["finite_ind_clause"]
    )  # number of words,number of independent finite clauses (T-units)
    index_dict["dc_c"] = safe_divide(
        index_dict["finite_dep_clause"], index_dict["finite_clause"]
    )  # number of dependent clauses,number of finite clauses (dependent clauses per clause)

    index_dict["ccomp_c"] = safe_divide(
        index_dict["finite_compl_clause"], index_dict["finite_clause"]
    )  # complement clauses,number of finite clauses
    index_dict["relcl_c"] = safe_divide(
        index_dict["finite_relative_clause"], index_dict["finite_clause"]
    )  # relative clauses,number of finite clauses
    index_dict["infinitive_prop"] = safe_divide(
        index_dict["to_clause"], index_dict["all_clauses"]
    )  # infinitive clauses,total number of clauses
    index_dict["nonfinite_prop"] = safe_divide(
        index_dict["nonfinite_clause"], index_dict["all_clauses"]
    )  # nonfinite clauses,total number of clauses

    return index_dict


def output_vertical(
    list_text, outname, ordered_output="simple", prettyp=False
):  # list version of parsed text, list of attributes to output, name of output file
    ...
    for sent_id, sent in enumerate(list_text):
        if (
            len(sent) < 1
        ):  # for some reason, there are some blank "sentences" in the output (at the end of each document)
            continue  # skip these
        if sent_id != 0:
...
def output_xml(list_text, outname=False, xml_element=None):
    ...
    for sent_id, sent in enumerate(list_text):
        sent_level = ET.SubElement(
            xml_element, "sentence", attrib={"sent_id": str(sent_id)}
        )
        sent_text = ET.SubElement(
            sent_level, "sentence_text"
        )  # add xml tag for sentence text
        sentence_list = []
...
def LGR_Full(
    filenames, outname, indices_dict=INDEX_LIST, cats_d=cats, outdirname="", output=None
):  # output options should be list ["xml","vertical"]
    outf = open(outname, "w")  # create output file
    ...
    if (
        type(filenames) == str
    ):  # allow filenames to be a list OR a target folder name/glob search
        if (
            "*" not in filenames and filenames[-1] != "/"
        ):  # if users forgot to include the "/"
    ...
    for filename in filenames:
        simple_fname = filename.split("/")[
            -1
        ]  # grab the filename without all preceding folders
        print(simple_fname)
        ...
        for x in indices_dict:
            if x in ["nwords", "wrd_length"]:
                ...
                output_list.append(
                    str((tag_output[x] / tag_output["nwords"]) * 10000)
                )  # normed by 10,000 words
        outf.write("\n" + ",".join(output_list))
        ...
        if output != None:
            ...
            if "xml" in output:
                xmloutname = (
                    xmldir + "".join(simple_fname.split(".")[:-1]) + ".xml"
                )  # format filename
                output_xml(tag_output["tagged_text"], xmloutname, xml_element=None)
            # output vertical
            if "vertical" in output:
                vertoutname = (
                    vertdir + "".join(simple_fname.split(".")[:-1]) + ".tsv"
                )  # format filename
                output_vertical(
                    tag_output["tagged_text"], vertoutname, ordered_output="full"
...
def calcFromXml(xml_filename, indices_dict=INDEX_LIST):
    ...
    for tags in root.iter(
        "biber_tags"
    ):  # all biber tags are listed as tag attributes (e.g., <biber_tags main_tag="verb" spec_tag1="non_past_tense" semantic_tag1="mental_verb"/>)
    ...
    for upos in root.iter(
        "UPOS"
    ):  # all pos are listed as text within tags (e.g., <UPOS>NOUN</UPOS>)
    ...
    for filename in filenames:
        ...
        for x in INDEX_LIST:
            if x in ["nwords", "wrd_length"]:
                ...
                output_list.append(
                    str((tagDict[x] / tagDict["nwords"]) * 10000)
                )  # normed by 10,000 words
        outf.write("\n" + ",".join(output_list))
    outf.flush()
    outf.close()
...
def LGR_XML(xml_files, outname, INDEX_LIST, cats):  # for processing TMLE xml texts
    ...
    refined_INDEX_LIST = [
        x for x in INDEX_LIST if x not in ignore_list
    ]  # ignores raw counts for complexity items
    outf.write(
        "filename,learning_environment,mode,discipline,subdiscipline,text_type,"
        + ",".join(refined_INDEX_LIST)
    )  # write header
    ...
    def discipline_fixer(discipline):
        typo_dict = {
            ...
            "natual_science": "natural_science",
            ...
            "pnatural_science": "natural_science",
            "n/a": "service_encounters",
        }
    ...
    for filename in xml_files:
        ...
        pre_tt = "\t".join(
            [le, root[0].attrib["mode"], cleaner(root[0].attrib["file_type"])]
        )
        output_list = [
            simple_fname,
            le,
            root[0].attrib["mode"],
            discipline_fixer(root[0].attrib["discipline"]),
            sdp,
            tt_dict[pre_tt],
        ]  # start list for indices
        ...
        for x in refined_INDEX_LIST:
            if x in no_norming:
                ...
                output_list.append(
                    str((output[x] / output["nwords"]) * 10000)
                )  # normed by 10,000 words
        outf.write("\n" + ",".join(output_list))
    ...
    for filename in xml_files:
        ...
        if root[1].attrib["text_type"] in ["plain_text", "plaintext"]:
            ...
            if len(root) < 3:  # this is due to a problem. Some texts have some issue
                ...
                text = root[2].text
        ex_sents += sent_exampler(
            LGR_Analysis(text, INDEX_LIST, cats, output=True)["tagged_text"], target
        )
